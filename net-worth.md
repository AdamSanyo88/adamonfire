  ...
  chart=new Chart(ctx,{
    ...
    options:{
      ...
      plugins:{
        ...
        tooltip:{callbacks:{title:it=>"Percentilis "+it[0].label,label:it=>"Küszöb €"+Number(it.raw).toLocaleString()}}
      },
      scales:{
...
function highlight(p){
  if(!chart)return;
  const idx=100-p; const colors=new Array(100).fill("#cfd8dc"); colors[idx]="#0d6efd";
  chart.data.datasets[0].backgroundColor=colors; chart.update();
}
...
function compute(){
  ...
  document.querySelectorAll("#rows tr[data-type]").forEach(r=>{
    ...
    let n=0;
    if(t==="prop")n=v-d;else if(t==="inv"||t==="asset")n=v;else if(t==="liab")n=-d;
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  document.getElementById("rows").addEventListener("click",e=>{
    if(e.target.tagName==="BUTTON"&&e.target.closest("tr").dataset.type==="prop"){e.target.closest("tr").remove();compute();}
  });
});
</script>
